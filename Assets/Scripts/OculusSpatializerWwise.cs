/************************************************************************************
Filename    :   OculusSpatializerWwise.cs
Content     :   Interface into real-time geometry reflection engine for Wwise
...
public class OculusSpatializerWwise : MonoBehaviour
{
    ...
    void Start()
    {
        OSP_Wwise_AssignRaycastCallback(AudioRaycast, System.IntPtr.Zero);
    }

    void OnDestroy()
    {
        OSP_Wwise_AssignRaycastCallback(System.IntPtr.Zero, System.IntPtr.Zero);
    }
    ...
    void Update()
    {
        ...
        OSP_Wwise_SetDynamicRoomRaysPerSecond(raysPerSecond);
        OSP_Wwise_SetDynamicRoomInterpSpeed(roomInterpSpeed);
        OSP_Wwise_SetDynamicRoomMaxWallDistance(maxWallDistance);
        OSP_Wwise_SetDynamicRoomRaysRayCacheSize(rayCacheSize);

        gLayerMask = layerMask;
        OSP_Wwise_UpdateRoomModel(1.0f);
        ...
        if (visualizeRoom)
        {
            ...
            Vector3 pos;
            OSP_Wwise_GetRoomDimensions(dims, coefs, out pos);
            ...
            OSP_Wwise_GetRaycastHits(points, normals, HIT_COUNT);
    ...
    private const string strOSP = "OculusSpatializerWwise";

    [DllImport(strOSP)]
    private static extern int OSP_Wwise_AssignRaycastCallback(AudioRaycastCallback callback, System.IntPtr data);
    [DllImport(strOSP)]
    private static extern int OSP_Wwise_AssignRaycastCallback(System.IntPtr callback, System.IntPtr data);

    [DllImport(strOSP)]
    private static extern int OSP_Wwise_SetDynamicRoomRaysPerSecond(int RaysPerSecond);
    [DllImport(strOSP)]
    private static extern int OSP_Wwise_SetDynamicRoomInterpSpeed(float InterpSpeed);
    [DllImport(strOSP)]
    private static extern int OSP_Wwise_SetDynamicRoomMaxWallDistance(float MaxWallDistance);
    [DllImport(strOSP)]
    private static extern int OSP_Wwise_SetDynamicRoomRaysRayCacheSize(int RayCacheSize);
    [DllImport(strOSP)]
    private static extern int OSP_Wwise_UpdateRoomModel(float wetLevel); // call from main thread!!
    [DllImport(strOSP)]
    private static extern int OSP_Wwise_GetRoomDimensions(float[] roomDimensions, float[] reflectionsCoefs, out Vector3 position);
    [DllImport(strOSP)]
    private static extern int OSP_Wwise_GetRaycastHits(Vector3[] points, Vector3[] normals, int length);
}
